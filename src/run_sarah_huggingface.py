...
my_model_checkpoint = 'C:\\Users\\CShuwen\\Desktop\\nlp_Project\\CS4248_G10\\bert\\bert'
...
def test():
    ...
    test_set = squad.data
    print("data", test_set)
    validation_set = squad.get_test_set()['validation']
    print("===validation set===\n", validation_set)

    validation_set.set_format(type=validation_set.format["type"], columns=list(validation_set.features.keys()))
    print("===updated validation set===\n", validation_set)

    # makes prediction with the validation dataset
    ...
    with open(CWD + "/result/start_logit.txt", 'w') as file:
        ...
        for i in range(0, 2, 1):
            ...
            res += "]\n"
            par = test_set["validation"][i]
            file.write(f'{par["context"]}\n')
            file.write(res)

        file.close()

    with open(CWD + "/result/end_logit.txt", 'w') as file:
    ...
    final_predictions = dict(
        postprocess_qa_predictions(test_set["validation"], validation_set, predictions.predictions))
